<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      console.log("Loaded Suites:", suites);
    } catch (error) {
  ...
  async function fetchLatestReport() {
    const response = await fetch("http://localhost:3001/latest-report");
    const data = await response.json();
  ...
  function copyToTestID(id) {
    console.log("Selected Test ID:", id);
    testID = id;
  }

  function copyToSuiteID(id) {
  ...
  <div class="card bg-base-300 rounded-box grid flex-grow place-items-center w-1/2 p-4 mr-2 ml-4">
    <div class="card-body items-center text-center">
      ...
      <label class="form-control w-full max-w-xs mt-4">
        <input type="text" class="input input-bordered w-full max-w-xs" bind:value={testID} placeholder="Enter test ID" />
      </label>
    ...
    <pre bind:this={outputRef} class="bg-black rounded-box p-4 w-full overflow-auto whitespace-pre-wrap h-64 max-h-64">{output}</pre>
  </div>
      ...
      <div class="mt-4">
        ...
        {console.log(suite)}
          <div class="collapse bg-base-200 mb-4">
            ...
            <label for="collapse-{suiteIndex}" class="collapse-title font-medium justify-start cursor-pointer">
            ...
            <div class="collapse-content">
              <button class="btn btn-active btn-ghost btn-xs" on:click={() => copyToSuiteID(Array.isArray(suite.suiteId) ? suite.suiteId[0] : suite.suiteId)}>
                Select Suite
              </button>
                          ...
                          <th>
                            {#if Array.isArray(test.id)}
                              {#each test.id as testId}
                                <button class="btn btn-active btn-ghost btn-xs mr-1" on:click={() => copyToTestID(testId)}>
                                  {testId}
                                </button>
                              {/each}
                            {:else}
                              <button class="btn btn-active btn-ghost btn-xs" on:click={() => copyToTestID(test.id)}>
                                {test.id}
                              </button>
                            {/if}
                          </th>
